...
parser.add_argument('--model_id', default='damo/speech_campplus_sv_zh-cn_16k-common', type=str, help='Model id in modelscope')
...
def get_spk_emb(wav, feature_extractor, embedding_model, device, save=True):
...
def get_spk_emb_t(wav, feature_extractor, embedding_model, device, save=True):
    feat = feature_extractor(wav).unsqueeze(0).to(device)
    with torch.no_grad():
        embedding = embedding_model(feat).detach().squeeze(0)
    return embedding

def get_spk_sim(wav1, wav2, feature_extractor, embedding_model, device):
    embedding1 = get_spk_emb(wav1, feature_extractor, embedding_model, device)
    embedding2 = get_spk_emb(wav2, feature_extractor, embedding_model, device)
...
def build_model():
    ...
    return feature_extractor, embedding_model, device
